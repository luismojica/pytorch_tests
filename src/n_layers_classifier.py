...
# Luis Mojica
# UTD
# n-layerd network classifier Using PyTorch
...
X, Y = make_classification(n_samples=n_samples,n_features=n_features, n_redundant=0, n_informative=n_features, n_classes=n_classes,random_state=1)
...
# 
class nLayerdNetwork(nn.Module):
	"""docstring for nLayerdNetwork"""
	def __init__(self, input_size, n_classes):
		super(nLayerdNetwork, self).__init__()
		self.linear1 = nn.Linear(input_size,n_classes)
		self.linear2 = nn.Linear(input_size,n_classes)
		self.softmax = nn.Softmax()

		# an affine operation: y = Wx + b
		self.fc1 = nn.Linear(input_size, int(input_size*1.5))
		self.fc2 = nn.Linear(int(input_size*1.5), n_classes)
...
# Try the model out:
model = nLayerdNetwork(n_features,n_classes)
